...
jwt.verify(refreshToken , process.env.REFRESH_TOKEN_SECRET , (err , payload)=>{
  ...
  const sellerId = payload._id
  const accessToken =  jwt.sign({ _id: sellerId}, process.env.ACCESS_TOKEN_SECRET, { expiresIn: "60s" });
  const refreshToken =  jwt.sign({ _id: sellerId }, process.env.REFRESH_TOKEN_SECRET, { expiresIn: "1y" });
...
async function sellerSignup(req, res, next) {
  try {
    ...
    const existSeller = await Seller.findOne({ email: Result.email.toLowerCase() });
    if (existSeller)
    ...
    const isExist = await Seller.findOne({ email: Result.email });
    if (isExist) {
...
async function sendOtp(req, res, next) {
  try {
    ...
    const existSeller = await Seller.findOne({ email: Result.email });
    if (existSeller) {
      const otp = `${Math.floor(1000 + Math.random() * 9000)}`;
      const otpExpiry = Date.now() + 120000;
      transporter.sendMail({
